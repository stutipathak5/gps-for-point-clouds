...
from manifold_matern.utils import (
    export_fun,
    construct_mesh_graph,
...
if __name__ == "__main__":
    ...
    if args.eigenpairs_file is None:
        ...
        eigenpairs = get_eigenpairs(
            mesh, V, num_eigvals=num_eigenpairs, eps_target=1e-10
        )
    else:
        print("Reading eigenpairs from a file")
        eigenpairs_ = np.load(args.eigenpairs_file)
        eigenpairs = (eigenpairs_[:, 0], eigenpairs_[:, 1:])
    ...
    gp.sigma_f.constrain_bounded(1.0, 100000.0)
    ...
    # variance=np.squeeze(variance[None,:])
    # print(variance.shape)
    # print(variance[variance >= 0].shape)
    # print(np.where(variance < 0))
    variance = abs(variance)
    # variance=np.append(variance[0:100178], 0.05)
    # print(variance.shape)
    ...
    export_fun(
        os.path.join(output_dir, "0_groud_truth.pvd"),
        convert_to_firedrake_function(V, ground_truth),
    )
    export_fun(
        os.path.join(output_dir, "0_mean.pvd"), convert_to_firedrake_function(V, mean)
    )
    export_fun(
        os.path.join(output_dir, "0_std.pvd"), convert_to_firedrake_function(V, std)
    )

    for s in range(len(samples)):
        export_fun(
            os.path.join(output_dir, "0_sample_%d.pvd" % s),
    ...
    if args.mayavi and mayavi_installed:
        ...
        vmin, vmax = -4.0, 4.0
        plot_example(
            os.path.join(output_dir, "1_ground_truth.png"),
            mesh,
            V,
            ground_truth,
            X,
        ...
        plot_example(
            os.path.join(output_dir, "1_mean.png"), mesh, V, mean, X, vmin, vmax
        )
        plot_example(
            os.path.join(output_dir, "1_std.png"),
            mesh,
            V,
            std,
            X,
        ...
        for s in range(len(samples)):
            plot_example(
                os.path.join(output_dir, "1_sample_%d.png" % s),
                mesh,
                V,
                samples[s],
                X,
